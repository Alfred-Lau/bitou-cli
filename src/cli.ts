...
const commands = [
  ...
  // 测试页面SEO
  {
    ...
    option: [
      ...
      ["--precheck", "预热对应的url", ""],
      ["--postcheck", "后检相关事项 ", ""],
      ["--checkStatusCode301", "检查301", ""],
      ["--checkStatusCode404", "检查404", ""],
    ],
...
export default function main() {
  ...
  commands.forEach((command) => {
    const p = program.command(command.name).description(command.description);
    ...
    p.argument(command.argument[0], command.argument[1]).action(command.action);
  });
